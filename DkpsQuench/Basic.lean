/-
The scaffold of this file was generated by Aristotle (https://aristotle.harmonic.fun).
...
/-
Formalization of the DKPS / Quench query-efficiency paper, Theorem 2.

Main results:
* `Theorem2_part1_paper` — the NN estimator has `MSE(ŷ_NN) ≤ ε` with high
  probability (Part 1), and `Theorem2_part2_paper` — query efficiency relative
  to the baseline `ŷ_Q` (Part 2).  Both take the embedding-concentration event
  `h_conc` (paper Theorem 1) and the model-coverage event `h_cover`
  (Assumption 2) as explicit high-probability hypotheses.
* `Theorem2_part1_paper_subevent` / `Theorem2_part2_paper_subevent` — the same
  conclusions from a measurable high-probability *sub-event* of the
  embedding-error event (so the error event itself need not be measurable).
* `DkpsQuench.AcharyyaBridge.Theorem2_part2_of_aligned_spectral` — the version
  that DISCHARGES `h_conc`/`h_conc_meas` from the Acharyya2025 spectral
  concentration chain.

Proof skeleton: `step1`–`step7` (Lipschitz → triangle → concentration → cover →
pointwise error → parameter choice → MSE), the high-probability-event helpers
`HighProbAtTop.inter`/`inter_v3`/`mono_eventually`, and the nearest-neighbor
estimator `hNN_estimator`/`nnIndex`/`nnIndex_isArgmin`.
-/
import Mathlib
