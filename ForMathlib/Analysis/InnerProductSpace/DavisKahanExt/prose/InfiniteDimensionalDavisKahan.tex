...
The efficient development is not to prove the entire infinite-dimensional
scaffold before returning to the finite theory.  The first target is a small
bounded Hilbert-space core: projection algebra, two-projection geometry,
operator angles, graph subspaces, direct rotation, the ordered operator-norm
Sylvester theorem, and the ordered $\sin\Theta$ and reflection
$\sin 2\Theta$ estimates.  These arguments are basis free and should be
specialized back to finite dimensions.

Finite-dimensional singular-value lists, arbitrary finite unitarily invariant
norms, Frobenius identities, eigenvalue-order combinatorics, and statistical
corollaries should remain in the finite development.  Waiting for infinite
symmetric ideals before closing those results would be inefficient.
