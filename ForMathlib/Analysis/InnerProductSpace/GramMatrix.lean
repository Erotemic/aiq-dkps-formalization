...
* `ForMathlib.exists_linearIsometry_span_map_eq_of_inner_eq`: the **span-to-span
  core** — for families `φ : ι → E`, `ψ : ι → F` in two (possibly different) inner
  product spaces over `𝕜`, equal pairwise inner products give a linear isometry
  `span 𝕜 (range φ) →ₗᵢ span 𝕜 (range ψ)` sending each `φ i` to `ψ i`.  No
  finiteness is needed.
* `ForMathlib.exists_linearIsometry_map_eq_of_inner_eq`: the span-to-ambient
  corollary — the same isometry composed with the inclusion `span 𝕜 (range ψ) ↪ F`.
...
the span of the `φ i` to the span of the `ψ i` with `L (φ i) = ψ i` for every `i`.
...
(`ker Tφ ≤ ker Tψ`) and norm preserving, and it lands in `span 𝕜 (range ψ)` (the
range of `ψ`'s linear-combination map).
-/
theorem exists_linearIsometry_span_map_eq_of_inner_eq {φ : ι → E} {ψ : ι → F}
  ...
  have hf'_isom : ∀ s : (LinearMap.range Tφ), ‖f' s‖ = ‖s‖ := by
    ...
    exact hf_isom s
  set Lr : (LinearMap.range Tφ) →ₗᵢ[𝕜] (LinearMap.range Tψ) := ⟨f', hf'_isom⟩ with hLr
  ...
  refine ⟨(LinearIsometryEquiv.ofEq _ _ hrangeψ).toLinearIsometry.comp
      (Lr.comp (LinearIsometryEquiv.ofEq _ _ hrangeφ).symm.toLinearIsometry), fun i => ?_⟩
  ...
  show ((LinearIsometryEquiv.ofEq _ _ hrangeψ)
      (Lr ((LinearIsometryEquiv.ofEq _ _ hrangeφ).symm ⟨φ i, _⟩)) : F) = ψ i
  rw [LinearIsometryEquiv.coe_ofEq_apply, htransφ]
  show ((f' ⟨φ i, hmemRφ⟩ : LinearMap.range Tψ) : F) = ψ i
  rw [hf', LinearMap.codRestrict_apply]
  exact hfφ

/--
**Gram rigidity, span-to-ambient form.** The span-to-span core
