...
* `ForMathlib.exists_linearIsometry_of_inner_eq`: the **span-level core** — equal
  pairwise inner products give a linear isometry from `span 𝕜 (range φ)` into the
  ambient space sending each `φ i` to `ψ i`.  No finiteness is needed.
...
/--
**Gram rigidity, span-level core.** If two families `φ ψ : ι → E` of vectors in an
inner product space have equal pairwise inner products
`⟪φ i, φ j⟫ = ⟪ψ i, ψ j⟫`, then there is a linear isometry `L` from the span of
the `φ i` into `E` with `L (φ i) = ψ i` for every `i`.

No finiteness of `ι` or of `E` is required.  The map is built as `φ i ↦ ψ i` on
`span 𝕜 (range φ)` (the range of `φ`'s linear-combination map); equality of inner
products makes it well defined (`ker Tφ ≤ ker Tψ`) and norm preserving.
-/
theorem exists_linearIsometry_of_inner_eq {φ ψ : ι → E}
    (h : ∀ i j, ⟪φ i, φ j⟫_𝕜 = ⟪ψ i, ψ j⟫_𝕜) :
    ∃ L : (Submodule.span 𝕜 (Set.range φ)) →ₗᵢ[𝕜] E,
  ...
  set Lr : (LinearMap.range Tφ) →ₗᵢ[𝕜] E := ⟨f, hf_isom⟩ with hLr
