...
── remaining (v10) ─────────────────────────────────────────────
OP1 ✅, OP2 ✅, G2.1 ✅  (Opus, v10 — done, axiom-clean)
OP3.0 (Fable, next slot) ─→ OP3.A (Opus) ─→ OP3.B (Opus)
G2.0 gate (Fable) ─→ G2.2 (Fable)
G3.0 gate (Fable) ─→ G3.(ii),(iii) (Fable);  G3 d=1 descope (Opus, AFTER G3.0)
(F3-annex: optional, anytime)
```

All fully-routed Opus work is done.  The only remaining Opus items are
**OP3.A/OP3.B** (blocked on Fable's OP3.0) and the **G3 d=1 descope** (blocked
on Fable's G3.0 gate — the tanΘ statement shape must be fixed first; the sinΘ
single-vector bound is certain but off-spec for G3).  So the ball is with
Fable: OP3.0 then G3.0/G2.0.
...
|------|------|------|-----------|----------|
| 1 | G3 | Subspace tanΘ (graph operator, similar-to-symmetric Sylvester) | 5/5 | **Fable**; statement-risk; d=1 descope waits on G3.0 gate (sinΘ d=1 is certain, tanΘ d=1 is source-dependent) |
