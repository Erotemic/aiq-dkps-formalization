...
/-- Paper Assumption 3 (as stated): image of each learned decision function is closed, bounded, complete. -/
...
lemma loss_uniformly_bounded (n d d' : ℕ)
    ...
    ∃ M : ℝ, ∀ u, ∀ᵐ ω ∂(Measure.pi (fun _ : Fin (n + 1) => P)),
      |loss (learn (fun i => (psi_hat u ω (Fin.castSucc i), (ω (Fin.castSucc i)).2)) (psi_hat u ω (Fin.last n))) (ω (Fin.last n)).2| ≤ M := by
        ...
        exact hM ( X u ω, Y ω ) ⟨ h_pred, hω_label ⟩
...
lemma ContinuousLoss.continuousLossInPred {d' : ℕ} {loss : LossFunction d'}
  ...
  intro y
  -- h : Continuous fun p : Y×Y => loss p.1 p.2
...
lemma BoundedLearningRule.isBounded_range (n d d' : ℕ) {learn : LearningRule n d d'}
    ...
    ∀ t, Bornology.IsBounded (Set.range (learn t)) := by
  rcases h with ⟨K, hKc, hK⟩
  intro t
  have hsub : Set.range (learn t) ⊆ K := by
    intro y hy
...
theorem consistency_transfer_dkps_bayes (d d' : ℕ)
  ...
  -- just instantiate your existing theorem with L = bayesRisk ...
...
## Spaces
* Paper: embeddings `ψ ∈ ℝ^d`                      → Lean: `E d`
* Paper: labels/covariates `y ∈ ℝ^{d'}`           → Lean: `Y d'`

## Data
* Paper: training set `Tₙ = ((ψ₁,y₁),…,(ψₙ,yₙ))`   → Lean: `Fin n → E d × Y d'`
* Paper: joint sample of size `n+1`               → Lean: `ω : Fin (n+1) → E d × Y d'`
* Paper: i.i.d. sampling from `P_{ψY}`            → Lean: product measure `Measure.pi (fun _ : Fin (n+1) => P)`

## Learning rule and loss
* Paper: learning rule `h(·;Tₙ)`                  → Lean: `LearningRule n d d'`
* Paper: loss `ℓ : ℝ^{d'}×ℝ^{d'} → ℝ_{≥0}`        → Lean: `LossFunction d' := Y d' → Y d' → ℝ`
  (We allow `ℝ` for convenience; nonnegativity can be added as an extra assumption if desired.)

## Risks
* Paper risk (Def. A.1): `R_ℓ(P_{ψY}, h(·;Tₙ))`
    = `E[ℓ(h(ψ_{n+1};Tₙ), y_{n+1})]`
  → Lean: `risk n d d' P learn loss`

* Paper estimated-embedding risk: `R_ℓ(P_{ψY}, h(·; \hat Tₙ))`
  → Lean: `risk_est n d d' P learn loss psi_hat`

## Main theorems
* Paper Theorem 1 (fixed n): convergence of estimated-embedding risk to true-embedding risk
  → Lean: `risk_converges_fixed_n` (and the paper-facing `theorem1_*` below)

* Paper Theorem 2: “consistency transfer” via a diagonal/schedule argument
  → Lean: `consistency_transfer_dkps` (and the paper-facing `theorem2_*` below)
-/

namespace Helm2025Paper
...
These are *definitionally equal* to the core definitions, but named to mirror the paper.
-/

abbrev EmbeddingSpace (d : ℕ) := E d
abbrev LabelSpace (d' : ℕ) := Y d'
abbrev LearnRule (n d d' : ℕ) := LearningRule n d d'
abbrev Loss (d' : ℕ) := LossFunction d'

/-- Paper risk `R_ℓ(P_{ψY}, h(·;Tₙ))`. -/
abbrev R (n d d' : ℕ) (P : Measure (E d × Y d')) (learn : LearningRule n d d') (loss : LossFunction d') : ℝ :=
  risk n d d' P learn loss

/-- Paper estimated-embedding risk `R_ℓ(P_{ψY}, h(·;\hat Tₙ))`. -/
abbrev Rhat (n d d' : ℕ) (P : Measure (E d × Y d')) (learn : LearningRule n d d') (loss : LossFunction d')
    (psi_hat : (Fin (n + 1) → E d × Y d') → Fin (n + 1) → E d) : ℝ :=
  risk_est n d d' P learn loss psi_hat

/-!
## Paper Assumptions A1–A4 (as standalone predicates)

We keep the **core** assumptions as the canonical ones used in proofs, and provide “paper-shaped”
variants where helpful for exposition.
-/

/-- Paper A4: continuity in the first argument, pointwise in `y`. -/
def A4_ContinuousLossInPred (d' : ℕ) (loss : LossFunction d') : Prop :=
  ∀ y : Y d', Continuous (fun yhat : Y d' => loss yhat y)

/-- Core A4 (joint continuity) implies paper A4 (pointwise continuity in prediction). -/
lemma ContinuousLoss.to_A4 {d' : ℕ} {loss : LossFunction d'}
    (h : ContinuousLoss d' loss) : A4_ContinuousLossInPred d' loss := by
  intro y
  have h' : Continuous (fun p : Y d' × Y d' => loss p.1 p.2) := by
    simpa [ContinuousLoss] using h
  -- compose with yhat ↦ (yhat, y)
...
/--
Paper A3 (as stated in Appendix A.1): for each training set `Tₙ`, the image of `h(·;Tₙ)` is
closed, bounded, and complete.

We record it here as a predicate.  The core development assumes a *uniform compact-range*
condition `BoundedLearningRule`, which is convenient for dominated-convergence style arguments
(see comments in the core file).
-/
def A3_ImageClosedBoundedComplete (n d d' : ℕ) (learn : LearningRule n d d') : Prop :=
  ∀ t : Fin n → E d × Y d',
    IsClosed (Set.range (learn t)) ∧
    Bornology.IsBounded (Set.range (learn t)) ∧
    IsComplete (Set.range (learn t))

/-- Core A3 (`BoundedLearningRule`) implies the “bounded” part of paper A3. -/
lemma BoundedLearningRule.isBounded_range (n d d' : ℕ) {learn : LearningRule n d d'}
    (h : BoundedLearningRule n d d' learn) :
    ∀ t, Bornology.IsBounded (Set.range (learn t)) := by
  rcases h with ⟨K, hKc, hK⟩
...
/-!
## Paper Theorem 1 (fixed n)

Paper statement (paraphrase, matching the Lean formulation):

For fixed `n`, under A1–A4 and DKPS alignment consistency, the risk using estimated embeddings
converges (as the estimation budget grows) to the risk using true embeddings.

In Lean, the estimation budget is indexed by `u : ℕ` (paper uses `(m,r) → ∞` or a single index `u`).
-/

/-- Paper Theorem 1, in “Tendsto” form (directly the core theorem). -/
theorem theorem1_fixed_n
    (n d d' : ℕ)
    (P : Measure (E d × Y d')) [IsProbabilityMeasure P]
    (learn : LearningRule n d d')
    (loss : LossFunction d')
    ...
    (h_cont_loss : ContinuousLoss d' loss)
    (h_bound_label : BoundedLabelSupport d d' P) :
    Tendsto (fun u => risk_est n d d' P learn loss (psi_hat u)) atTop (𝓝 (risk n d d' P learn loss)) :=
  risk_converges_fixed_n n d d' P learn loss psi_hat
    h_meas_psi h_align h_inv h_cont_learn h_bound_learn h_cont_loss h_bound_label

/-- Paper Theorem 1, in the common “absolute difference → 0” form. -/
theorem theorem1_fixed_n_abs
    (n d d' : ℕ)
    (P : Measure (E d × Y d')) [IsProbabilityMeasure P]
    (learn : LearningRule n d d')
    (loss : LossFunction d')
    ...
    (h_cont_loss : ContinuousLoss d' loss)
    (h_bound_label : BoundedLabelSupport d d' P) :
    Tendsto (fun u => |risk_est n d d' P learn loss (psi_hat u) - risk n d d' P learn loss|)
      atTop (𝓝 0) := by
  have h :=
    theorem1_fixed_n (n:=n) (d:=d) (d':=d') (P:=P) (learn:=learn) (loss:=loss) (psi_hat:=psi_hat)
      h_meas_psi h_align h_inv h_cont_learn h_bound_learn h_cont_loss h_bound_label
  have h' : Tendsto (fun u => risk_est n d d' P learn loss (psi_hat u) - risk n d d' P learn loss)
      atTop (𝓝 0) := by
    simpa [sub_self] using
      (h.sub
        (tendsto_const_nhds :
          Tendsto (fun _ : ℕ => risk n d d' P learn loss)
            atTop (𝓝 (risk n d d' P learn loss))))
  simpa using h'.abs

/-!
## Bayes risk and the paper’s “consistency” notion

Appendix A uses Bayes risk
`R^*_\ell(P_{ψY}, H) = inf_{h∈H} R_\ell(P_{ψY}, h)`.

The core file’s Theorem 2 is written with a generic target limit `L : ℝ`.
Here we package the paper-facing instantiation `L := BayesRisk`.
-/

/-- Risk of a (deterministic) decision function `h : E → Y`. -/
def risk_df (d d' : ℕ) (P : Measure (E d × Y d'))
    (loss : LossFunction d') (h : E d → Y d') : ℝ :=
  ∫ p, loss (h p.1) p.2 ∂P

/-- Bayes risk over a hypothesis class `H`. -/
def bayesRisk (d d' : ℕ) (P : Measure (E d × Y d'))
    (loss : LossFunction d') (H : Set (E d → Y d')) : ℝ :=
  sInf ((fun h => risk_df (d:=d) (d':=d') P loss h) '' H)

/-- “Expected-risk consistency” (the version used in the core Theorem 2). -/
def ConsistentExpected (d d' : ℕ) (P : Measure (E d × Y d'))
    (loss : LossFunction d')
    (learn : (n : ℕ) → LearningRule n d d')
    (L : ℝ) : Prop :=
  Tendsto (fun n => risk n d d' P (learn n) loss) atTop (𝓝 L)

/-- Expected-risk consistency to Bayes risk over `H`. -/
def ConsistentExpectedToBayes (d d' : ℕ) (P : Measure (E d × Y d'))
    (loss : LossFunction d')
    (learn : (n : ℕ) → LearningRule n d d')
    (H : Set (E d → Y d')) : Prop :=
  ConsistentExpected d d' P loss learn (bayesRisk d d' P loss H)

/-- Paper Theorem 2 (expected-risk version): consistency transfers along a schedule `φ(n) → ∞`. -/
theorem theorem2_consistency_transfer_expected
    (d d' : ℕ)
    (P : Measure (E d × Y d')) [IsProbabilityMeasure P]
    (learn : (n : ℕ) → LearningRule n d d')
    (loss : LossFunction d')
    ...
    (h_cont_loss : ContinuousLoss d' loss)
    (h_bound_label : BoundedLabelSupport d d' P)
    (h_consistent : ConsistentExpected d d' P loss learn L) :
    ∃ phi : ℕ → ℕ, Tendsto phi atTop atTop ∧
      Tendsto (fun n => risk_est n d d' P (learn n) loss (psi_hat n (phi n)))
        atTop (𝓝 L) := by
  -- This is exactly the core theorem (just with the `ConsistentExpected` alias).
  simpa [ConsistentExpected] using
    consistency_transfer_dkps (d:=d) (d':=d') (P:=P) (learn:=learn) (loss:=loss) (psi_hat:=psi_hat)
      (L:=L) h_meas_psi h_align h_inv h_cont_learn h_bound_learn h_cont_loss h_bound_label h_consistent

/-- Paper Theorem 2 instantiated with `L = BayesRisk` (Bayes-consistency transfer). -/
theorem theorem2_consistency_transfer_bayes
    (d d' : ℕ)
    (P : Measure (E d × Y d')) [IsProbabilityMeasure P]
    (learn : (n : ℕ) → LearningRule n d d')
    (loss : LossFunction d')
    ...
    (h_cont_loss : ContinuousLoss d' loss)
    (h_bound_label : BoundedLabelSupport d d' P)
    (h_consistent : ConsistentExpectedToBayes d d' P loss learn H) :
    ∃ phi : ℕ → ℕ, Tendsto phi atTop atTop ∧
      Tendsto (fun n => risk_est n d d' P (learn n) loss (psi_hat n (phi n)))
        atTop (𝓝 (bayesRisk d d' P loss H)) := by
  -- instantiate `theorem2_consistency_transfer_expected` with `L := bayesRisk ...`
  simpa [ConsistentExpectedToBayes, ConsistentExpected] using
    theorem2_consistency_transfer_expected (d:=d) (d':=d') (P:=P) (learn:=learn) (loss:=loss)
      (psi_hat:=psi_hat) (L:=bayesRisk d d' P loss H)
      h_meas_psi h_align h_inv h_cont_learn h_bound_learn h_cont_loss h_bound_label
      (by simpa [ConsistentExpectedToBayes, ConsistentExpected] using h_consistent)

/-!
## Suggested sanity checks (cheap, fast, and usually non-hanging)

`#find` can sometimes be slow/hang in large developments; these tend to be reliable:
...
#check consistency_transfer_dkps
#check Helm2025Paper.theorem1_fixed_n_abs
#check Helm2025Paper.theorem2_consistency_transfer_bayes
```

If you *do* want to use `#find`, it often helps to:
* restrict the search by giving more structure in the pattern,
* temporarily set `set_option maxHeartbeats 0` (careful: can make IDE unresponsive),
* or search for a lemma name in the mathlib docs and `#check` it directly.
-/
