...
This project request had uuid: ea5a99ad-8fc4-4212-baf8-8271c0461a99
...
/-- A “nearest neighbor index” condition in the estimated DKPS space. -/
def IsNNIndex
  (ψHat : ℕ → Ω → Model Q X → Vec d)
  (refs : ∀ n : ℕ, Ω → Fin (n+1) → Model Q X)
  (n : ℕ) (ω : Ω) (f : Model Q X) (i_star : Fin (n+1)) : Prop :=
  ∀ i : Fin (n+1),
    ‖ψHat n ω (refs n ω i_star) - ψHat n ω f‖ ≤ ‖ψHat n ω (refs n ω i) - ψHat n ω f‖

/--
`hNN` is *the* DKPS+NN estimator: it returns the full-benchmark score of a nearest neighbor
(reference model closest to the target in estimated DKPS coordinates).
-/
def IsDKPSNN
  (Qstar : Finset Q)
  (score : Model Q X → Finset Q → ℝ)
  (ψHat : ℕ → Ω → Model Q X → Vec d)
  (refs : ∀ n : ℕ, Ω → Fin (n+1) → Model Q X)
  (hNN : ℕ → Ω → Model Q X → ℝ) : Prop :=
  ∀ n ω f, ∃ i_star : Fin (n+1),
    IsNNIndex (Q := Q) (X := X) (d := d) ψHat refs n ω f i_star ∧
    hNN n ω f = score (refs n ω i_star) Qstar

/-- A fixed-Q⋆ Lipschitz assumption (simpler than your `LipschitzScore` which is Qsub-parametric). -/
def LipschitzScoreFull
  (Qstar : Finset Q)
  (score : Model Q X → Finset Q → ℝ)
  (γ : ℝ)
  (ψ : Model Q X → Vec d) : Prop :=
  ∀ f f', |score f Qstar - score f' Qstar| ≤ γ * ‖ψ f - ψ f'‖

lemma norm_sub_le_three {V : Type*} [SeminormedAddCommGroup V]
    (a b c : V) : ‖a - c‖ ≤ ‖a - b‖ + ‖b - c‖ := by
  -- a - c = (a - b) + (b - c)
  simpa [sub_add_sub_cancel] using (norm_add_le (a - b) (b - c))

/-
Core deterministic lemma used in Quench: under a “good DKPS error event”
and existence of a close true neighbor among references, NN in estimated space
is close in true space.
-/
lemma nn_true_dist_bound
  ...
  (i_star : Fin (n+1))
  (c eps' : ℝ)
  (hNN : IsNNIndex (Q := Q) (X := X) (d := d) ψHat refs n ω f i_star)
  (h_ref : ∀ i : Fin (n+1), ‖ψHat n ω (refs n ω i) - ψ (refs n ω i)‖ ≤ c)
  (h_tgt : ‖ψHat n ω f - ψ f‖ ≤ c)
  (h_close : ∃ j : Fin (n+1), ‖ψ (refs n ω j) - ψ f‖ ≤ eps') :
  ‖ψ (refs n ω i_star) - ψ f‖ ≤ eps' + 4*c := by
  classical
  rcases h_close with ⟨j, hj⟩

  -- Flip the reference bounds when needed
  have h_ref_rev : ∀ i : Fin (n+1),
      ‖ψ (refs n ω i) - ψHat n ω (refs n ω i)‖ ≤ c := by
    intro i
    simpa [norm_sub_rev] using (h_ref i)

  have h_tgt_rev : ‖ψ f - ψHat n ω f‖ ≤ c := by
    simpa [norm_sub_rev] using h_tgt

  -- Step 1: estimated distance to j is ≤ eps' + 2c
  have h_est_j :
      ‖ψHat n ω (refs n ω j) - ψHat n ω f‖ ≤ eps' + 2*c := by
    have h1 :=
      norm_sub_le_three
        (ψHat n ω (refs n ω j)) (ψ (refs n ω j)) (ψHat n ω f)

    have h2 :=
      norm_sub_le_three
        (ψ (refs n ω j)) (ψ f) (ψHat n ω f)

    have h12 :
        ‖ψHat n ω (refs n ω j) - ψHat n ω f‖
          ≤ ‖ψHat n ω (refs n ω j) - ψ (refs n ω j)‖
            + (‖ψ (refs n ω j) - ψ f‖ + ‖ψ f - ψHat n ω f‖) := by
      refine le_trans h1 ?_
      -- add the bound for `‖ψ (refs j) - ψHat f‖` on the right
      have := add_le_add_left h2 ‖ψHat n ω (refs n ω j) - ψ (refs n ω j)‖
      simpa [add_assoc, add_left_comm, add_comm] using this

    have hbound :
        ‖ψHat n ω (refs n ω j) - ψHat n ω f‖ ≤ c + (eps' + c) := by
      refine le_trans h12 ?_
      have := add_le_add (h_ref j) (add_le_add hj h_tgt_rev)
      simpa [add_assoc, add_left_comm, add_comm] using this

    -- rewrite `c + (eps' + c)` to `eps' + 2*c`
    -- either `ring` or `ring_nf` works
    have : c + (eps' + c) = eps' + 2*c := by ring
    simpa [this] using hbound

  -- Step 2: NN property gives estimated distance to i_star ≤ estimated distance to j
  have h_est_i_star :
      ‖ψHat n ω (refs n ω i_star) - ψHat n ω f‖ ≤ eps' + 2*c :=
    le_trans (hNN j) h_est_j

  -- Step 3: convert estimated bound into true bound: eps' + 4c
  have h3 :=
    norm_sub_le_three
      (ψ (refs n ω i_star)) (ψHat n ω (refs n ω i_star)) (ψ f)

  have h4 :=
    norm_sub_le_three
      (ψHat n ω (refs n ω i_star)) (ψHat n ω f) (ψ f)

  have h34 :
      ‖ψ (refs n ω i_star) - ψ f‖
        ≤ ‖ψ (refs n ω i_star) - ψHat n ω (refs n ω i_star)‖
          + (‖ψHat n ω (refs n ω i_star) - ψHat n ω f‖
            + ‖ψHat n ω f - ψ f‖) := by
    refine le_trans h3 ?_
    have := add_le_add_left h4 ‖ψ (refs n ω i_star) - ψHat n ω (refs n ω i_star)‖
    simpa [add_assoc, add_left_comm, add_comm] using this

  have hfinal :
      ‖ψ (refs n ω i_star) - ψ f‖ ≤ c + ((eps' + 2*c) + c) := by
    refine le_trans h34 ?_
    have := add_le_add (h_ref_rev i_star) (add_le_add h_est_i_star h_tgt)
    simpa [add_assoc, add_left_comm, add_comm] using this

  -- rewrite `c + ((eps' + 2*c) + c)` into `eps' + 4*c`
  have : c + ((eps' + 2*c) + c) = eps' + 4*c := by ring
...
theorem theorem1_from_prior_work {m p d : ℕ}
  ...
  contrapose! hEig;
  intro h;
  obtain ⟨ lambda₁, lambda_d, C₁, C₂, hC₁, hC₂, hlambda₁, hlambda_d, hlambda ⟩ := h;
  obtain ⟨ v, hv₁, hv₂ ⟩ := hlambda₁ 0 |>.1;
...
theorem Theorem2_part1
  ...
  (refs : ∀ n : ℕ, Ω → Fin (n+1) → Model Q X)
  (hNN : ℕ → Ω → Model Q X → ℝ)
  (hNN_spec : IsDKPSNN (Q := Q) (X := X) (d := d) Qstar score ψHat refs hNN)
  (γ : ℝ) (hγ : 0 < γ)
  (hLip : LipschitzScoreFull (Q := Q) (X := X) (d := d) Qstar score γ ψ)
  -- plus: DKPS concentration + support/denseness assumptions (not shown here)
  :
  ∀ ε : ℝ, 0 < ε →
  ...
  intro ε hε
  -- proof should follow paper:
  -- 1) use DKPS concentration → small `c(n)` w.h.p.
  -- 2) use support/denseness → existence of close reference neighbor w.h.p.
  -- 3) apply `nn_true_dist_bound` + Lipschitz → pointwise error bound
  -- 4) integrate to MSE and conclude high probability
  sorry
...
/--
Part 2 is a corollary of Part 1:
if `hNN` achieves arbitrarily small MSE w.h.p. and `hQ` is bounded below by some `c>0`
(eventually, uniformly in `ω`), then w.h.p. `MSE(hNN) ≤ MSE(hQ)` eventually.
-/
theorem Theorem2_part2_of_part1
  {Ω : Type} [MeasurableSpace Ω]
  (Qstar : Finset Q)
  (score : Model Q X → Finset Q → ℝ)
  (Pf : Measure (Model Q X)) [IsProbabilityMeasure Pf]
...
/--
Theorem 2, Part 2 (query-efficiency): DKPS+NN dominates a baseline whose MSE is bounded away from 0.

This is just `Theorem2_part2_of_part1` applied to `Theorem2_part1`.
-/
theorem Theorem2_part2
  ...
  refine Theorem2_part2_of_part1 (Q := Q) (X := X) (Ω := Ω)
    (Qstar := Qstar) (score := score) (Pf := Pf)
    (μ := μ) (hμ := hμ) (hNN := hNN) (hQ := hQ)
    ?hNN_small hQ_pos
  intro ε hε
  exact Theorem2_part1 (Q := Q) (X := X) (Ω := Ω)
    (Qstar := Qstar) (score := score) (Pf := Pf)
    (μ := μ) (hμ := hμ)
    (d := d) (ψ := ψ) (ψHat := ψHat) (refs := refs)
    (hNN := hNN) (hNN_spec := hNN_spec)
    (γ := γ) (hγ := hγ) (hLip := hLip)
    ε hε
