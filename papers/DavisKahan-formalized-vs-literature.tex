...
Sylvester bound that underlies the dimension-free operator-norm theorem.  A
closing section records what remains --- the operator-norm $\sin\Theta$
assembly, the canonical principal-angle API, and unitarily invariant norms ---
ranked by difficulty, each with a worked proof route.
\end{abstract}
...
are formalized in a phase-free product form.  What is still missing is the
dimension-free operator-norm $\sin\Theta$ theorem (its analytic engine, an
operator Sylvester bound, is already in place; only the compression assembly
remains), a packaged canonical principal-angle API, the subspace-level
...
Two items remain, plus two deliberately deferred ones.  They are listed
hardest first; each comes with the proof route that has been checked on paper,
so that the remaining work is implementation rather than route discovery.
Lean-level detail (exact hypotheses, file placement, pitfalls) is maintained
in \texttt{dev/davis-kahan-gap-closure-plan.md} (v5) in the repository.
...
\item \emph{Canonical principal-angle API (difficulty 3/5, with one 2.5/5
  prerequisite).}  Define $\cos\Theta$ as the singular values of the (already
  formalized) flat overlap operator, and prove: values in $[0,1]$ (done),
  symmetry in the two subspaces, and the bridges to the overlap sum, the
  projector distance, and the $\mathrm{sqSinAngle}$ of the intertwining-unitary
  file.  Symmetry needs $\sigma(M^{*})=\sigma(M)$, which Mathlib lacks; in the
  only case needed (square $M$) it follows from
  $MM^{*}=U(M^{*}M)U^{*}$ with $U$ the kernel-completed unitary polar factor
  --- already formalized --- plus ``unitary conjugation preserves sorted
  eigenvalues,'' a Courant--Fischer consequence.
