...
and per-eigenvector $\sin2\theta$ theorem, and the operator Sylvester bound
that underlies the dimension-free operator-norm theorem.  A closing section
records what remains --- the operator-norm $\sin\Theta$ assembly, the
canonical principal-angle API, the $\tan2\theta$ variant, and unitarily
invariant norms --- ranked by difficulty, each with a worked proof route.
\end{abstract}

\vspace{1.5em}
% NOTE: Update permalink if underlying proof changes, so this always is up to date wrt to the referenced lean file.
\noindent Companion note to \href{https://github.com/AIQ-Kitware/aiq-dkps-formalization/blob/b9eee75734410c152d5e45b4e6a29ccda179cfaa/ForMathlib/Analysis/InnerProductSpace/DavisKahan.lean}{ForMathlib/Analysis/InnerProductSpace/DavisKahan.lean} and its sibling files under \href{https://github.com/AIQ-Kitware/aiq-dkps-formalization/tree/b9eee75734410c152d5e45b4e6a29ccda179cfaa/ForMathlib/Analysis/InnerProductSpace}{ForMathlib/Analysis/InnerProductSpace/}, part of a larger formalization effort at \href{https://github.com/AIQ-Kitware/aiq-dkps-formalization}{github.com/AIQ-Kitware/aiq-dkps-formalization}.
...
substrate, and Davis's per-eigenvector $\sin2\theta$ theorem is formalized in
a phase-free product form.  What is still missing is the dimension-free
operator-norm $\sin\Theta$ theorem (its analytic engine, an operator Sylvester
bound, is already in place; only the compression assembly remains), a packaged
canonical principal-angle API, the $\tan2\theta$ variant and the
subspace-level $\sin2\Theta$ theorem, and general unitarily invariant norm
versions. The purpose of this note is to explain
...
Three items remain, plus two deliberately deferred ones.  They are listed
...
\item \emph{Per-eigenvector $\tan2\theta$ theorem (difficulty 2/5).}  Add the
  vanishing-pinch hypotheses (the diagonal blocks of $H$ vanish on $U$ and
  $U^{\perp}$) to the formalized $\sin2\theta$ setting; the formalized key
  identity collapses to
  $cs(\alpha-\beta)=(c^2-s^2)\,\mathrm{re}\inner{q}{Hp}$ with
  $|\mathrm{re}\inner{q}{Hp}|\le\norm{H}_{\op}$ directly --- no rotation trick
  --- giving $(b-a)\,cs\le|c^2-s^2|\,\norm{H}_{\op}$, i.e.\
  $\tan2\theta\le2\norm{H}_{\op}/(b-a)$.  Pure assembly on the already-factored
  identity lemma.
\end{enumerate}
