...
and the operator Sylvester bound that underlies the dimension-free
operator-norm theorem.  A closing section records what remains --- the
operator-norm $\sin\Theta$ assembly, the canonical principal-angle API, the
$\sin2\Theta$/$\tan2\Theta$ family, and unitarily invariant norms --- ranked by
difficulty, each with a worked proof route.
\end{abstract}
...
the singular-subspace extension are formalized, and Davis's sharper 1963
total-rotation theory is formalized together with its polar-decomposition
substrate.  What is still missing is the dimension-free operator-norm
$\sin\Theta$ theorem (its analytic engine, an operator Sylvester bound, is
already in place; only the compression assembly remains), a packaged canonical
principal-angle API, the $\sin2\Theta$ and $\tan2\Theta$ theorems, and general
unitarily invariant norm versions. The purpose of this note is to explain
...
Four items remain, plus two deliberately deferred ones.  They are listed
...
\item \emph{Subspace-level $\sin2\Theta$/$\tan2\Theta$ (difficulty 5/5 ---
  deferred).}  A warning for future attempts: summing the per-eigenvector
  $\sin2\theta$ bound of item~\ref{it:sin2theta} over an eigenbasis does
...
\item \emph{Per-eigenvector $\sin2\theta$ theorem (difficulty 4/5 --- in
  progress, assigned).}\label{it:sin2theta}  Davis's sharp two-subspace
  estimate: if the quadratic form of $T$ is $\ge b$ on an invariant subspace
  $U$ and $\le a$ on $U^{\perp}$, $x$ is a unit eigenvector of $T+H$ (its
  eigenvalue \emph{unconstrained}), and $P=P_U$, then in product form
  \[
    (b-a)\,\norm{Px}\,\norm{x-Px}\;\le\;\norm{H}_{\op},
    \qquad\text{i.e.}\quad \sin2\theta\le\frac{2\norm{H}_{\op}}{b-a}.
  \]
  The checked route needs \emph{no complex phase alignment}, the classical
  sore point: writing $c=\norm{Px}$, $s=\norm{x-Px}$, $p=Px/c$, $q=(x-Px)/s$,
  the two eigen-equations paired with $p$ and $q$ subtract to the real
  identity
  \[
    cs\,(\alpha-\beta)
    =cs\,\bigl(\mathrm{re}\inner{Hq}{q}-\mathrm{re}\inner{Hp}{p}\bigr)
     +(c^2-s^2)\,\mathrm{re}\inner{q}{Hp},
  \]
  ($\alpha=\mathrm{re}\inner{Tp}{p}\ge b$,
  $\beta=\mathrm{re}\inner{Tq}{q}\le a$) because
  $\mathrm{re}(c^2w-s^2\bar w)=(c^2-s^2)\,\mathrm{re}\,w$ identically.  The
  right-hand side is then bounded by $\norm{H}_{\op}$ by testing $H$ against
  the two rotated unit vectors $\gamma p+\sigma q$ and $-\sigma p+\gamma q$
  with the \emph{real} half-angle choice $\gamma^2=(1-2cs)/2$,
  $\sigma^2=(1+2cs)/2$, $2\gamma\sigma=c^2-s^2$.
...
\item \emph{Per-eigenvector $\tan2\theta$ theorem (difficulty 2/5, after
  item~\ref{it:sin2theta}).}  Add the vanishing-pinch hypotheses (the diagonal
  blocks of $H$ vanish on $U$ and $U^{\perp}$); the identity of
  item~\ref{it:sin2theta} collapses to
