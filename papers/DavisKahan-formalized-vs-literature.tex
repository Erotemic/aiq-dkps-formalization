...
Sylvester bound that underlies the dimension-free operator-norm theorem, and a
canonical principal-angle API.  A closing section records what remains --- the
operator-norm $\sin\Theta$ assembly and unitarily invariant norms --- each with
a worked proof route.
\end{abstract}
...
API is packaged.  What is still missing is the dimension-free operator-norm
$\sin\Theta$ theorem (its analytic engine, an operator Sylvester bound, is
already in place; only the compression assembly remains), the subspace-level
$\sin2\Theta$ theorem, and general unitarily invariant norm versions. The
purpose of this note is to explain
...
assumptions.  Only the compression assembly on top of it remains
(item~\ref{it:opnorm} below).
...
One item remains, plus two deliberately deferred ones.  They are listed
hardest first; the remaining item comes with the proof route that has been
checked on paper, so that the remaining work is implementation rather than
route discovery.  Lean-level detail (exact hypotheses, file placement,
pitfalls) is maintained in \texttt{dev/davis-kahan-gap-closure-plan.md} (v5) in
the repository.
...
\item \emph{Operator-norm $\sin\Theta$ assembly (difficulty 3/5).}\label{it:opnorm}
  The headline $\norm{\widehat Q\,P}_{\op}\le\norm{S-T}_{\op}/\gap$ (that
  operator norm \emph{is} $\norm{\sinT}_{\op}$; the principal-angle
  identification can come later).  Route: compress to
  $X\colon v\mapsto P_{U}v$ from the trailing $S$-span to the leading
  $T$-span; invariance and the commutation $TP_U=P_UT$ give the Sylvester
  relation $T_U X-X S_V=(\text{compressed }S-T)$; the quadratic-form
  hypotheses of the separated Sylvester bound are exactly the Courant--Fischer
  form bounds on spectral spans; un-compressing and taking adjoints yields the
  theorem.  All ingredients exist; this is plumbing.
\end{enumerate}
