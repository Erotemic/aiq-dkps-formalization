...
Since this note was first written, \emph{Davis's sharper total-rotation
estimate} (his Theorem~3.2) has also been formalized, in a separate chain of
files: the refinement
\[
...
(his Theorem~4.1, \texttt{EigenvalueChange.lean}).  The supporting machinery —
the positive square root, a partial-isometry API, the operator polar
decomposition $A=U\lvert A\rvert$ (with a $\mathbb C$/\texttt{CFC.abs}
headline), and Davis's \S2 canonical intertwining unitary of two complete
orthogonal projection families — lives in \texttt{PositiveSqrt.lean},
\texttt{PartialIsometry.lean}, \texttt{PolarDecomposition.lean},
\texttt{IntertwiningUnitary.lean}; the rotation theorems themselves are in
\texttt{RotationBound.lean} (all under
\texttt{ForMathlib/Analysis/InnerProductSpace/}).

The remaining DK-family results, and the machinery each would require, are as
follows.
