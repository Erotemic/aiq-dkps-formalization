...
`|λₖ(Â) − λₖ(A)| ≤ ‖Â − A‖op` for symmetric matrices, eigenvalues sorted.
Check what Mathlib has around `Matrix.IsHermitian.eigenvalues` /
Courant–Fischer min-max; if absent, prove min-max first (finite-dimensional,
self-contained; strong Mathlib-contribution candidate). Risk: Mathlib's
eigenvalues are unsorted (indexed by an orthonormal basis) — sorting/min-max
plumbing is the real cost here.
